...
function addArtist(array, nName, nYears, nGenre, nNationality, nBio) {
  array[array.length] = {
    name: nName,
    years: nYears,
    genre: nGenre,
...
//console.log(addArtist(artists, "Brunch Grah", "1988-present", "Modern Flatulism", "American", "Did some stuff and some things. Wouldn't recommend it."))
...
function artistInfo(array, nName){
  for (let i = 0; i < array.length; i++){
    if (array[i].name === nName){
        return array[i].bio
    }
  }
}
...
function artistByCountry(array, nNationality){
  const arr = []
  for (let i = 0; i < array.length; i++){
    if (array[i].nationality === nNationality){
      arr.push(array[i].name)
    }
  }
  return arr
